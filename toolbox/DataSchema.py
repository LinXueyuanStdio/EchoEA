#!/usr/bin/env python
# -*- coding: utf-8 -*-
# 读取数据集，数据预处理，数据缓存，数据增强
# outline
# 1. utils function
#   - cache_data(data, cache_path: Union[str, Path])
#   - read_cache(cache_path: Union[str, Path]) -> Any
#   - read_ids_and_names(file_path: Union[str, Path], sp="\t")
#   - read_triple_hrt(file_path: Union[str, Path]) -> List[Tuple[int, int, int]]
#   - read_triple_htr2hrt(file_path: Union[str, Path]) -> List[Tuple[int, int, int]]
#   - save_triple_hrt(triples: List[Tuple[int, int, int]], file_path: Union[str, Path])
#   - append_align_triple(triple: List[Tuple[int, int, int]], entity_align_list: List[Tuple[int, int]])
#   - build_map_tr_h(triplets)
#   - build_map_hr_t(triplets)
# 2. relational triplet data
#   - RelationalTripletData(dataset_name: str,
...
from toolbox.DatasetSchema import (
...
class RelationalTripletData(BaseData):
    """ The class is the main module that handles the knowledge graph.
        ...
        Examples:
            >>> from toolbox.DataSchema import RelationalTripletData
            >>> from toolbox.DatasetSchema import FreebaseFB15k
            >>> dataset = FreebaseFB15k()
